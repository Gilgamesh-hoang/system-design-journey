...
5. **Practical Technical Deep Dive**: When explaining theoretical concepts, always include technical implementation details, under-the-hood mechanics, real-world tools/deployment, and operational costs/risks. Avoid purely abstract answers.
...
5. **Notion Updates**: You are ONLY allowed to update Notion pages when the user EXPLICITLY requests it. Do not automatically update Notion on your own. When recording notes (e.g., in the "Note của tôi" block), ALWAYS structure them hierarchically by "Chặng" (Phase) and use Toggle blocks (`{toggle="true"}`) for main chapters and sections to keep the page clean and easy to navigate, matching the original DDIA template's structure.
...
6. **Pre-explain Terminology**: Before presenting the questions, briefly explain any complex or important terminology (e.g., Write Amplification) that will appear in the questions to ensure the user has the necessary context.
7. **Detailed Post-Quiz Explanation**: After the user answers the questions, provide a detailed breakdown explaining why the correct answers are right and why the other options are incorrect.
8. **Scope**: Apply these rules ONLY when testing theory knowledge, not during practical Katas or Flagship coding tasks.
